...
> **`fc` = "full cycle":** ejecuta el ciclo entero de una (rama → commits → enlace →
> cerrar sobre `main` → push + borrar rama → `./deploy.sh`), sin ir preguntando entre pasos.
> **Menos en el enlace: ahí para y espera.** `fc` ahorra preguntas, no la revisión.
