...
const trash = load("wp_discarded"),
  fav = load("wp_fav"), // "interesantes": sí preliminar del swipe
  star = load("wp_star"); // "favoritos": ascendidos desde interesantes (tras la IA). 3 cubos exclusivos; "sin ver" = ninguno
...
function hydrateEstado() {
  return fetch("/estado" + qsPerfil())
    ...
    .then((e) => {
      ...
      for (const k of star) if (trash.has(k)) star.delete(k);
      for (const k of fav) if (trash.has(k) || star.has(k)) fav.delete(k);
      blockSel.clear();
      ...
      localStorage.setItem("wp_star", JSON.stringify([...star]));
      localStorage.setItem("wp_excl", JSON.stringify(exclMap));
      localStorage.setItem("wp_catexcl", JSON.stringify(catExclMap));
...
let headers = [],
  data = [],
  sortKeys = [],
  view = ""; // view: '' mazo | 'trash' papelera | 'fav' interesantes | 'star' favoritos
let iId = -1,
  iUrl = -1,
  iTitulo = -1,
  iPrecio = -1;
...
function fillCard(el, r) {
  ...
  add(
    "li-flags",
    where
      ? `${conEnvio ? "Con envío" : "Sin envío"}, ${where}`
      : conEnvio
        ? "Con envío"
        : "Sin envío",
  );
  ...
  if (view !== "" && stamp[key(r)])
    add(
      "li-when" + (view === "fav" || view === "star" ? " fav" : ""),
      `${view === "star" ? "Favorito" : view === "fav" ? "Interesante" : "Rechazado"} ${ago(stamp[key(r)])}`,
    );
...
function sortList(rows) {
  if (!listSort) {
    const order = [...(view === "trash" ? trash : view === "star" ? star : fav)]; // orden de llegada a la lista
...
function filteredRows() {
  const listView = view === "trash" || view === "fav" || view === "star";
  const q = listView ? norm(listQ) : ""; // el filtro solo aplica en vista de lista
  let rows = data.filter((r) => {
    const k = key(r);
    if (q && !norm(r[iTitulo] || "").includes(q)) return false;
    if (view === "trash" && listSeller && col(r, "vendedor") !== listSeller)
      return false;
    if (view === "trash") return trash.has(k);
    if (view === "fav") return fav.has(k);
    if (view === "star") return star.has(k);
    return !fav.has(k) && !trash.has(k) && !star.has(k) && !isExcluded(r); // mazo: sin clasificar y sin vetar (los lejos-sin-envío también entran)
  });
  if (listView)
...
function enforceLejos() {
  ...
  for (const r of data) {
    const k = key(r);
    if (!fav.has(k) && !trash.has(k) && !star.has(k) && isLejos(r)) {
      trash.add(k);
      stampNow(k);
      changed = true;
    }
...
function render() {
  ...
  $("#exportFav").hidden = !(view === "fav" && rows.length);
  const favConEnvio =
    view === "fav" && rows.some((r) => col(r, "envio") === "True");
  $("#calcPeso").hidden = !favConEnvio; // solo con interesantes con envío
  $("#priceNote").hidden = !favConEnvio; // la nota explica ese precio final: mismo criterio que el botón
  $("#listActions").hidden = !(view === "fav" && rows.length); // la sección solo existe con interesantes a la vista
...
function paintStat() {
  ...
  const clasif = (r) =>
    fav.has(key(r)) || trash.has(key(r)) || star.has(key(r)); // ya en algún cubo
  const favs = data.filter((r) => fav.has(key(r))).length;
  const stars = data.filter((r) => star.has(key(r))).length;
  ...
  const sinVer = data.length - favs - stars - disc - vetados; // "vistos" = favs + stars + disc; vetados salen aparte. Los lejos SÍ cuentan (están en el mazo); su línea es solo atajo para rechazarlos en bloque
  $("#stat").innerHTML =
    `<span><b>${sinVer}</b> sin ver</span>` +
    (vetados
      ? `<span><b>${vetados}</b> excluidos · <span class="link" id="trashExcl">mandar a rechazados</span></span>`
      : "") +
    (lejos
      ? `<span><b>${lejos}</b> de ellos lejos y sin envío · <span class="link" id="trashLejos">rechazar</span></span>`
      : "") +
    `<span><b>${disc}</b> rechazados ` +
    (disc || view === "trash"
      ? `· <span class="link" id="toggleTrash">${view === "trash" ? "volver" : "ver rechazados"}</span>`
      : "") +
    `</span>` +
    `<span><b>${favs}</b> interesantes ` +
    (favs || view === "fav"
      ? `· <span class="link" id="toggleFav">${view === "fav" ? "volver" : "ver interesantes"}</span>`
      : "") +
    `</span>` +
    `<span><b>${stars}</b> favoritos ` +
    (stars || view === "star"
      ? `· <span class="link" id="toggleStar">${view === "star" ? "volver" : "ver favoritos"}</span>`
      : "") +
    `</span>` +
    (sortKeys.length
  ...
  const t = $("#toggleTrash");
  if (t) t.onclick = toggle("trash");
  const f = $("#toggleFav");
  if (f) f.onclick = toggle("fav");
  const st = $("#toggleStar");
  if (st) st.onclick = toggle("star");
  const el = $("#trashLejos");
  if (el) el.onclick = trashLejos;
  const te = $("#trashExcl");
  if (te) te.onclick = trashExcluded;
  const cs = $("#clearSort");
...
function trashLejos() {
  const ks = data
    .filter(
      (r) =>
        !fav.has(key(r)) && !trash.has(key(r)) && !star.has(key(r)) && isLejos(r),
    )
    .map(key);
  if (!ks.length) return;
  ks.forEach((k) => {
...
function sellerCandidates() {
  ...
  for (const r of data) {
    ...
    else if (!fav.has(k) && !isExcluded(r) && !isLejos(r))
...
function blockSeller(s) {
  const newly = data
    .filter((r) => col(r, "vendedor") === s && !trash.has(key(r)))
    .map(key);
  blockSel.add(s);
  saveBlockSel();
  const wereStar = newly.filter((k) => star.has(k)); // para restaurar su cubo al deshacer
  newly.forEach((k) => {
    fav.delete(k);
    star.delete(k);
    trash.add(k);
...
function loadQuery(csv) {
  // carga el CSV (scopeado al perfil) y lo recuerda como el último de la persona
  fetch("/csvfile" + qsPerfil() + "&csv=" + encodeURIComponent(csv))
    .then((r) => r.text())
    .then((t) => loadCSV(t, csv));
...
function selectQuery(csv) {
  // input = solo el kw; el "desde" va como badge pino a la derecha
...
pick.oninput = () => {
  pickSince.hidden = true;
  qbox.classList.remove("has-since");
  openQlist();
...
function restoreLastCsv() {
  ...
  if (!last) return;
  refreshCsvs().then(() => {
    if (allQueries.some((q) => q.csv === last)) selectQuery(last);
  });
}
...
$("#scrape").onclick = async () => {
  ...
  stop.onclick = doStop; // restaura el estilo por si una parada previa se lo quitó
  function doStop() {
    // corta el scraper; el /scrape en curso vuelve con el CSV parcial ya escrito
    stop.onclick = null;
  ...
  const poll = setInterval(async () => {
    // el server responde /progress en paralelo al scrape
    try {
      const p = (
        await (
          await fetch(
            "/progress" +
              qsPerfil() +
              "&csv=" +
              encodeURIComponent(csvNameOf(kw, since)),
          )
        ).json()
      ).progress;
  ...
  try {
    ...
    snack("No se pudo buscar: " + e.message, null);
  } finally {
    clearInterval(poll);
    clearInterval(_timer);
    setLoading(false);
...
function renderSearches() {
  // relee del servidor y repinta con el filtro actual
  searchesList.innerHTML = '<div class="qempty">cargando…</div>';
  fetch("/searches" + qsPerfil())
    .then((r) => r.json())
    .then((list) => {
      allSearches = list;
      paintSearches();
    ...
    .catch(() => {
      searchesList.innerHTML = '<div class="qempty">no se pudo cargar</div>';
    });
}
...
function deleteSearch(csv, kw) {
  if (
    !confirm(
      `¿Borrar la búsqueda "${kw}"? Se pierde el CSV (el estado por perfil se conserva).`,
    )
  )
    return;
...
function afterCsvChange(oldCsv, newCsv) {
  ...
  if (curCsv === oldCsv) {
    if (newCsv) {
      selectQuery(newCsv);
      if (perfil) localStorage.setItem(lastCsvKey(), newCsv);
    } else {
      curCsv = null;
      pick.value = "";
      pickSince.hidden = true;
      ...
      $("#empty").textContent = "Busca algo primero";
      render();
    }
  }
}
...
const chip = $("#perfilChip"),
  gate = $("#gate");
const avatarEl = $("#perfilAvatar"),
  opts = $("#perfilOpts");
...
function hue(n) {
  let h = 0;
  for (const c of n) h = (h + c.charCodeAt(0) * 37) % 360;
  return h;
}
const colorOf = (p) => p.color || `hsl(${hue(p.name)} 85% 72%)`; // fallback: perfiles viejos sin color
const ink = (c) => {
  // texto legible según luminancia del fondo (sirve para pasteles nuevos y colores viejos oscuros)
  let L;
  if (c[0] === "#") {
    ...
    L = (parseFloat(c.match(/[\d.]+%/g)?.[1]) || 50) / 100;
  } // hsl(): 2º % = lightness
...
function setPerfil(name, color, isNew) {
  ...
  else knownPerfiles.push({ name, color });
  renderChip();
  closeGate();
  ...
  render();
  if (isNew) {
    // persiste ya el perfil vacío con su color
    for (const s of [trash, fav]) s.clear();
    fetch("/estado" + qsPerfil(), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ trash: [], fav: [], color }),
    }).catch(() => {});
...
function showPicker() {
  // fila de tarjetas + tile de añadir (si < 4)
  creator.hidden = true;
  tiles.hidden = false;
  tiles.innerHTML = "";
  for (const p of knownPerfiles) {
    ...
    b.querySelector(".edit").onclick = (e) => {
      e.stopPropagation();
      showCreator(p);
    };
...
function showCreator(edit) {
  // crear (edit=null) o editar un perfil: nombre + color
  editing = edit || null;
  tiles.hidden = true;
  ...
  pendingColor = editing
    ? colorOf(editing)
    : COLORS[knownPerfiles.length % COLORS.length];
  swatches.innerHTML = "";
...
function openGate(mode) {
  // 'first' (obligatorio) | 'switch' (se puede cerrar)
  $("#gateTitle").textContent =
    mode === "switch" ? "Cambiar de perfil" : "¿Quién está buscando?";
...
$("#creator").onsubmit = (e) => {
  ...
  if (!n) return;
  editing
    ? saveEdit(editing, n, pendingColor)
    : setPerfil(n, pendingColor, true);
};
  ...
  fetch("/perfil?perfil=" + encodeURIComponent(orig.name), {
    ...
    .then((res) => {
      ...
      if (wasActive) {
        perfil = res.name;
        perfilColor = color;
        localStorage.setItem("wp_perfil", perfil);
        renderChip();
      }
      reloadPicker();
    })
...
function deletePerfil(orig) {
  if (!confirm(`¿Borrar el perfil "${orig.name}"? Se perderá su estado.`))
    return;
  const wasActive = perfil === orig.name;
  fetch("/perfil?perfil=" + encodeURIComponent(orig.name), {
    method: "POST",
...
function reloadPicker() {
  // vuelve a leer la lista y repinta las tarjetas
  return fetch("/perfiles")
    .then((r) => r.json())
    .then((list) => {
      knownPerfiles = list;
      showPicker();
...
gate.onclick = (e) => {
  if (e.target === gate && !$("#gateX").hidden) closeGate();
}; // backdrop cierra solo en 'switch'
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && !$("#gateX").hidden) closeGate();
});
...
fetch("/perfiles")
  ...
  .then((list) => {
    ...
    if (me)
      setPerfil(me.name, colorOf(me), false); // dispositivo que ya sabe quién es -> directo
    else {
      perfil = "";
      renderChip();
      openGate("first");
    } // sin perfil -> chip vacío + elige/crea
  })
  .catch(() => {
    perfil
      ? setPerfil(perfil, `hsl(${hue(perfil)} 42% 40%)`, false)
      : (renderChip(), openGate("first"));
  });
...
function swUndo() {
  ...
  if (!h) return;
  if (h.wasFav) fav.add(h.k);
  else fav.delete(h.k);
  if (h.wasTrash) trash.add(h.k);
  else trash.delete(h.k);
...
function favText(rows) {
  const items = rows
    .map((r, i) => {
      const lines = [
        `${i + 1}. ${stripEmoji(col(r, "titulo"))} — ${pricePair(r)}`,
      ];
      const desc = col(r, "descripcion");
...
$("#copyFavOpt").onclick = (e) => {
  opts.open = false;
  copyFavs(e.currentTarget);
  ...
  try {
    ...
    snack(
      ok < 0
        ...
        : ok
          ? `Precio exacto de ${ok} artículo${ok === 1 ? "" : "s"}`
          : "Sin peso disponible",
      null,
    );
  } catch {
...
let rbArmed = false;
function gateOpen() {
  return gate.classList.contains("show") && !$("#gateX").hidden;
} // 'first' (obligatorio) no se cierra con atrás
function anyOpen() {
  return view !== "" || !searchesView.hidden || !swipeView.hidden || gateOpen();
}
function closeTop() {
