...
const PRICE_NOTE = 'El «precio para mí» es una estimación del coste final para el comprador ' +
...
function favText(rows) {
  const items = rows.map((r, i) => {
    const lines = [`${i + 1}. ${stripEmoji(col(r, 'titulo'))} — ${pricePair(r)}`];
...
function cardText(r) {
  ...
  return 'Este es un artículo de segunda mano de Wallapop que estoy pensando en comprar. ' + PRICE_NOTE +
